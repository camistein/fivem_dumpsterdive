...
const getClosestDumpster = (modelName:string, player:Player): Dumpster|null => {
  const hash = GetHashKey(modelName)
  const dumpster = GetClosestObjectOfType(player.coords.x, player.coords.y, player.coords.z, 1.0, hash, false,true,true)
      ...
      if(!state.player.nearDumpster) {
        state.settings.dumpsterModels.forEach((dumpModel) => {
          if(!!state.player && state.player !== null) { 
            ...
            if(!!foundDumpster && foundDumpster != null && isClose(state.player.coords, foundDumpster.coords)) {
         ...
         if(!!state.currentDumpster && state.currentDumpster !== null) {
            if(!DoesEntityExist(state.currentDumpster.id) || !isClose(state.player.coords, state.currentDumpster.coords)) {
              resetState();
            }
            else {
              if(!state.searching) {
                displayText(`${state.settings.translations?.dumpsterdive ?? "Dumpsterdive"}`, state.currentDumpster.coords, state.settings)
              }
              initDumpsterDive();
            }
         }
    ...
    if(!IsEntityDead(GetPlayerPed(-1))) {
      if(IsControlJustPressed(0, 23) || IsControlJustPressed(0, 38)) {
        if(!state.searching) {
          ...
          TaskStartScenarioInPlace(state.player.id, 'PROP_HUMAN_BUM_BIN', 0, true)
          SendNUIMessage({
...
on('__cfx_nui:dumpsterdive:Finished', (data: any, cb: (data:any) => void) => {
  if(!!state.currentDumpster && state.currentDumpster !== null) {
    emitNet("dumpsterdive:RecieveLoot", GetPlayerPed(-1), {dumpster: state.currentDumpster.uniqueId});
  }
  closeDumpsterDive();
  cb("ok")
});
